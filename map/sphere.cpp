...
#include<iostream>		//For Reading in the Map
#include<fstream>
...
void makeMapPoints(pt_type a, pt_type b, pt_type c, pt_type points[], int iterations)
{
/*   pt_type v1, v2, v3;
   if (iterations == 0)
      ...
      v2 = unit((a + c)/2);
      v3 = unit((b + c)/2);
      makeMapPoints(a ,v2, v1, points, iterations-1);
      makeMapPoints(c ,v3, v2, points, iterations-1);
      makeMapPoints(b ,v1, v3, points, iterations-1);
      makeMapPoints(v1 ,v2, v3, points, iterations-1);
   }*/
}
...
{
   //determine the size of the array based on the first two numbers in the
   //file given.  NOTE: for each triangle segment that is cut,
   //you will end up 4 smaller triangles segments with 3 pts per triangle.
	FILE *raw;
	int W,R;
	raw=fopen("honolulu_small.raw","r");//Open File
   ...
   pt_type *points = new pt_type[NumPoints];
	for(int i=0;i<W;i++){
		for(int j=0;j<R;j++){
			points[i+j].x=((i/W-.5)*2);
			points[i+j].y=(-j/R);
			fscanf(raw, "%f", &points[i+j].z); //Set the points array from file
			points[i+j].z=points[i+j].z/max;
		}
	}
   //int n=W*R;
   quadpoints=(NumPoints*2)-(2*R)-(2*W);

	int j=0;
   pt_type *tripoints = new pt_type[quadpoints];
   // Specifiy the vertices for a triangle
   for(int i=0;i<NumPoints-R;i+=2){
		tripoints[j]=points[i];
	//	fprintf(output,"%f", tripoints[j].x)
		tripoints[j+1]=points[i+1];
	//	fprintf(output,"%f", tripoints[j+1].x)
		tripoints[j+2]=points[i+R+1];
	//	fprintf(output,"%f", tripoints[j+2].x)
		tripoints[j+3]=points[i+R];
///		fprintf(output,"%f", tripoints[j+3].x)
		j+=4;
	}
   return tripoints;
}
...
void display( void )
{
   ...
   glClear( GL_COLOR_BUFFER_BIT | GL_DEPTH_BUFFER_BIT);     // clear the window
	for(int i=0; i<quadpoints; i+=4)
      glDrawArrays( GL_LINE_LOOP, i, 4 );    // draw the points
...
int main( int argc, char **argv )
{
   ...
   sphere_pts = generateMapPoints(iterations);
